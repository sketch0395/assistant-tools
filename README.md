...
| `check_url_legitimacy`                                                    | `tools/urlProvenance.js` | Website provenance/legitimacy checker: redirect resolution, punycode/homograph detection, RDAP domain-age lookup, VirusTotal + urlscan.io (sandboxed browser render) cross-reference (host injects its own API keys). |
